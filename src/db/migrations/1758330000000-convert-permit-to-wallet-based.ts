...
export class ConvertPermitToWalletBased1758330000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        // SupplierPermit 테이블 수정
        await queryRunner.query(`ALTER TABLE \`supplier_permits\` ADD \`supplier_address\` varchar(100) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`supplier_permits\` ADD \`buyer_address\` varchar(100) NULL`);
        
        // 기존 데이터 마이그레이션 (supplier_id -> supplier_address)
        ...
        // 기존 데이터 마이그레이션 (buyer_id -> buyer_address)
        await queryRunner.query(`
            UPDATE \`supplier_permits\` sp 
        ...
        // SupplierCap 테이블 수정
        await queryRunner.query(`ALTER TABLE \`supplier_caps\` ADD \`owner_address\` varchar(100) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`supplier_caps\` ADD \`supplier_address\` varchar(100) NOT NULL`);
        
        // 기존 데이터 마이그레이션 (owner_id -> owner_address)
        await queryRunner.query(`
        ...
        // EscrowAccount 테이블 수정
        await queryRunner.query(`ALTER TABLE \`escrow_accounts\` ADD \`supplier_address\` varchar(100) NOT NULL`);
